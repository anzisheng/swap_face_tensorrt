...
std::vector<float> FaceEmbdding_trt::detect(cv::Mat& srcimg,        std::vector<cv::Point2f>& face_landmark_5)
{
    ...
    std::string saveName = fmt::format("faceEmbedding_{}.jpg", g_token);

    imwrite(saveName, crop_img);
...
std::vector<float> FaceEmbdding_trt::postprocess(std::vector<float> &featureVector)
{
    ...
    std::string embeddingfile = fmt::format("embedding_{}.txt", g_token);
    ofstream destFile2(embeddingfile, ios::out); 
