...
Mat& affine_matrix, Mat& box_mask, samplesCommon::BufferManager &buffers)
{
    ...
    std::string save_name = fmt::format("swap_crop_{}.jpg", g_token);
    imwrite(save_name, crop_img);
    ...
    std::string save_name2 = fmt::format("swap_blur_{}.jpg", g_token);
    imwrite(save_name2, box_mask);
...
cv::Mat SwapFace_trt::verifyOutput(Mat &target_img, const samplesCommon::BufferManager& buffers, Mat& affine_matrix, Mat& box_mask)
{
    ...
    imwrite("swap_result.jpg", dstimg);    
    return dstimg;
}
